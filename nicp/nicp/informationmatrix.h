...
struct InformationMatrix : public Eigen::Matrix4f {
   ...
   */
  inline InformationMatrix(const Eigen::Matrix4f &other)
  ...
  /**
   *  This constructor creates an InformationMatrix using the input
   * Eigen::MatrixBase object.
   *  @param other is the input Eigen::MatrixBase object used to fill the
   * InformationMatrix.
   */
  template <typename OtherDerived>
  inline InformationMatrix(const Eigen::MatrixBase<OtherDerived> &other)
      : Eigen::Matrix4f(other) {
    block<1, 4>(3, 0).setZero();
    block<4, 1>(0, 3).setZero();
  }
  ...
  /**
   *  This methods implements the operator equal between an InformationMatrix
   * and an Eigen::MatrixBase object.
   */
  template <typename OtherDerived>
  inline InformationMatrix &operator=(
  ...
  /**
   *  This methods apply the input Eigen::MatrixBase transformation to the
   * InformationMatrix.
   *  @param other is the input Eigen::MatrixBase transformation to apply to the
   * InformationMatrix.
   *  @return the transformed InformationMatrix.
   *  @see transformInPlace()
   */
  template <typename OtherDerived>
  inline InformationMatrix transform(
   ...
   */
  template <typename OtherDerived>
  inline InformationMatrix &transformInPlace(
      const Eigen::MatrixBase<OtherDerived> &other) const {
    const Eigen::Matrix3f &R = other.block(0, 0, 3, 3);
 ...
 *
 *  This class provides a structure to handle creation and manipulation of a
 * vector of InformationMatrix.
 */
class InformationMatrixVector : public TransformableVector<InformationMatrix> {
 public:
  ...
  /**
   *  This methods apply the input transformation to each InformationMatrix
   * contained in the InformationMatrixVector.
   *  @param m is the input transformation to apply to the elements of the
  ...
  inline void transformInPlace(const OtherDerived &m) {
    ...
    for (size_t i = 0; i < size(); i++, t++) { *t = T * (*t) * Tt; }
  }
};
 ...
 */
typedef std::vector<Matrix6f, Eigen::aligned_allocator<Matrix6f> >
    Matrix6fVector;
